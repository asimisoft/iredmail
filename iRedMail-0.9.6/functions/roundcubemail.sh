...
rcm_plugin_enigma()
{
    ...
    # Create a symbol link to `/var/vmail`, so that it's easier for sysadmin
    # to know there's such an important directory.
    ln -s ${RCM_HTTPD_ROOT_SYMBOL_LINK}/plugins/enigma/home ${STORAGE_BASE_DIR}/pgp-keys >> ${INSTALL_LOG} 2>&1
